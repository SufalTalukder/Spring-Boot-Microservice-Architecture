...
public interface ProductAddToCartRepository extends JpaRepository<ProductAddToCartModel, Long> {
	...
	@Query(value = "SELECT product_id FROM add_to_cart_tbl WHERE add_to_cart_id = :eachCartId", nativeQuery = true)
	long findProductByCartId(@Param("eachCartId") long eachCartId);
}
